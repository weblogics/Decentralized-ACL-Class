	...
	*/
	...
	function setUser($identifier)
	{
		if(!empty($identifier))
		{
			...
			# Check our Relations
			$q1 = mysql_query("SELECT * FROM aclpermissions")or die(mysql_error());
			...
			# Check our Roles
			$q2 = mysql_query("SELECT * FROM aclroles WHERE identifier = '".$this->user."'")or die(mysql_error());
	...
	function setPermissions($role, $permissions)
	{
		...
		{
			# If So, Update them
			mysql_query("UPDATE ".$permTable." set roleId = '".$role."', permissions = '".$permissions."' ".$where)or die(mysql_error());
		}
		else
		{
			# Otherwise Create them
			mysql_query("INSERT INTO ".$permTable." (roleId, permissions) VALUES ('".$role."', '".$permissions."')")or die(mysql_error());
		}
	}
	...
	function setRole($uid, $roles)
	{
		...
		{
			# If So, Update them
			mysql_query("UPDATE ".$permTable." set identifier = '".$uid."', roles = '".$roles."' ".$where)or die(mysql_error());
		}
		else
		{
			# Otherwise Create them
			mysql_query("INSERT INTO ".$permTable." (identifier, roles) VALUES ('".$uid."', '".$roles."')")or die(mysql_error());
		}
	}
